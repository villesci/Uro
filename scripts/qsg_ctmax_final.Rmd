...
data_p1<-filter(data_p1,run_ID!="2")
data_p1$grams<-data_p1$weight

summary(glm(survival~grams*t5,data=data_p1,family="binomial"))
summary(glm(t5~survival,data=data_p1,family="gaussian"))

summary(glm(survival~grams,data=data_p1,family="binomial"))
ggplot(data=data_p1,aes(x=grams,y=survival))+geom_point()+stat_smooth(method="glm",method.args=list(family="binomial"))
...
#snail weight not really signficany between acclimations, but close
