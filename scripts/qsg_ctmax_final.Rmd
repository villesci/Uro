...
lt50s$lay1<-ifelse(lt50s$site=="BF",lay_bf,ifelse(lt50s$site=="FB",lay_fb,ifelse(lt50s$site=="GB",lay_gb,ifelse(lt50s$site=="HM",lay_hm,ifelse(lt50s$site=="OY",lay_oy,ifelse(lt50s$site=="TO",lay_to,ifelse(lt50s$site=="WH",lay_wh,NA)))))))

lt50s$lay2<-ifelse(lt50s$site=="BF",lay_bf2,ifelse(lt50s$site=="FB",lay_fb2,ifelse(lt50s$site=="GB",lay_gb2,ifelse(lt50s$site=="HM",lay_hm2,ifelse(lt50s$site=="OY",lay_oy2,ifelse(lt50s$site=="TO",lay_to2,ifelse(lt50s$site=="WH",lay_wh2,NA)))))))
```
...
#ggplot(data=all,aes(x=date,y=WTMP,color=site,group=site))+scale_color_viridis_d(name="Site",labels=c("Great Bay, NH","Woods Hole, MA","Humboldt, CA","Tomales,CA","Oyster, VA","Beaufort, NC","Folly Beach, SC"))+theme_classic()+ylab("SST (°C)")+xlab("Month")+scale_fill_discrete(name="Site",labels=c("Great Bay, NH","Woods Hole, MA","Humboldt, CA","Tomales,CA","Oyster, VA","Beaufort, NC","Folly Beach, SC"))+scale_x_date(date_labels = "%b")+ theme(axis.text.y=element_text(size=12),axis.title.x=element_text(size=22,vjust=-.1),axis.text.x=element_text(size=12),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),axis.title.y=element_text(size=22),legend.text=element_text(size=14),legend.title=element_text(size=22))+guides(color=guide_legend(override.aes=list(fill=NA)))+stat_summary(geom="line",fun.y=mean,size=2)
```
```{r,include=F}
...
"lay1"=lm(LT50~lay1,prot1),
"lay2"=lm(LT50~lay2,prot1),
"lay1.acc"=lm(LT50~lay1+acc,prot1),
"lay2.acc"=lm(LT50~lay2+acc,prot1),
"lay1*acc"=lm(LT50~lay1*acc,prot1),
"lay2*acc"=lm(LT50~lay2*acc,prot1))
...
mod1=(mods$'lay2*acc')#best fit model
summary(mod1)
summary(lm(LT50~lay2*acc,prot1))
...
"t.2"=lm(wt.max~lay1,prot1),
"u.2"=lm(wt.max~lay2,prot1),
"v.2"=lm(wt.max~lay1+acc,prot1),
"w.2"=lm(wt.max~lay2+acc,prot1),
"x.2"=lm(wt.max~lay1*acc,prot1),
"y.2"=lm(wt.max~lay2*acc,prot1))
